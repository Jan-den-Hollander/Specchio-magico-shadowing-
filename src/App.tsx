...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#080810] text-[#f5f0e8] font-sans selection:bg-[#c9a84c]/30">
      <div className="max-w-md mx-auto px-4 pb-20 relative z-10">
        ...
        <header className="text-center py-8">
          <motion.h1 
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            className="font-serif text-4xl font-light tracking-widest text-[#e8c97a] drop-shadow-[0_0_20px_rgba(201,168,76,0.3)]"
          >
            Specchio Magico
          </motion.h1>
          <p className="text-[0.65rem] tracking-[0.2em] uppercase text-[#c9a84c]/50 mt-2">
            Il tuo conversatore · Jouw Italiaanse partner
          </p>
        </header>

        {/* Mirror Section */}
        <div className="relative mx-auto w-full aspect-[3/4] max-w-[280px] mb-6">
          <div className="absolute inset-0 bg-gradient-to-br from-[#7a5810] via-[#c9a84c] to-[#5a3e08] rounded-[50%_50%_46%_46%_/_28%_28%_72%_72%] p-2 shadow-[0_20px_50px_rgba(0,0,0,0.8)]">
            <div className="w-full h-full bg-[#111128] rounded-[47%_47%_44%_44%_/_26%_26%_74%_74%] overflow-hidden relative">
              <video 
                ref={videoRef}
                autoPlay 
              ...
              <AnimatePresence>
                {!isCamOn && (
                  <motion.div 
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 bg-radial-at-center from-[#161630] to-[#080810]"
                  >
                    <Sparkles className="w-12 h-12 text-[#c9a84c] mb-4 animate-pulse" />
                    <p className="font-serif italic text-[#c9a84c]/80 text-sm">Activeer de spiegel om jezelf te zien</p>
                    <small className="text-[#c9a84c]/40 text-[0.6rem] mt-1 uppercase tracking-wider">Attiva lo specchio per vederti</small>
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          <button 
            type="button"
            onClick={(e) => { e.preventDefault(); toggleCam(); }}
            className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-[#080810] border border-[#c9a84c]/30 px-4 py-1.5 rounded-full text-[0.65rem] tracking-widest uppercase text-[#c9a84c]/70 hover:border-[#c9a84c] hover:text-[#c9a84c] transition-all flex items-center gap-2 whitespace-nowrap"
          >
            {isCamOn ? <CameraOff size={12} /> : <Camera size={12} />}
            {isCamOn ? 'Stop Spiegel' : 'Activeer Spiegel'}
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="space-y-1">
            <label className="text-[0.6rem] uppercase tracking-widest text-[#c9a84c]/50 ml-1 flex items-center gap-1">
              <Settings size={10} /> Niveau
            </label>
            <select 
              value={level}
              onChange={(e) => setLevel(e.target.value)}
              className="w-full bg-[#c9a84c]/5 border border-[#c9a84c]/20 rounded-lg px-3 py-2 text-xs outline-none focus:border-[#c9a84c]/40 transition-colors"
            >
              <option value="A1">Beginner (A1)</option>
              <option value="A2">Elementair (A2)</option>
              <option value="B1">Gevorderd (B1)</option>
              <option value="B2">Vloeiend (B2)</option>
            </select>
          </div>
          <div className="space-y-1">
            <label className="text-[0.6rem] uppercase tracking-widest text-[#c9a84c]/50 ml-1 flex items-center gap-1">
              <MessageSquare size={10} /> Onderwerp
            </label>
            <select 
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
        ...
        {/* Chat Area */}
        <div className="bg-black/20 border border-[#c9a84c]/10 rounded-2xl h-64 overflow-y-auto p-4 mb-6 space-y-4 scrollbar-thin scrollbar-thumb-[#c9a84c]/20">
          {messages.map((msg, i) => (
            <motion.div 
              key={i}
              initial={{ opacity: 0, y: 10 }}
            ...
            >
              <div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                ...
                {msg.role === 'model' ? (
                  <>
                    <span className="font-serif italic text-lg text-[#e8c97a] block mb-1">{msg.it}</span>
                    <span className="text-[0.7rem] text-white/40 block">{msg.nl}</span>
                    {msg.ph && <span className="text-[0.65rem] text-[#c9a84c]/40 italic block mt-1">/{msg.ph}/</span>}
                  </>
                ) : (
                  <>
                    <span>{msg.it}</span>
                    {msg.score !== undefined && (
                      <div className={`mt-2 text-[0.6rem] font-bold uppercase px-2 py-0.5 rounded-full inline-block ${
                        msg.score === 2 ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                        msg.score === 1 ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20' :
                        'bg-red-500/10 text-red-400 border border-red-500/20'
                      }`}>
          ...
          {isThinking && (
            <div className="flex gap-1.5 p-3 bg-[#c9a84c]/5 border border-[#c9a84c]/10 rounded-2xl rounded-bl-none w-16">
        ...
        <div className="flex items-center justify-center gap-8 mb-4">
          <div className="flex flex-col items-center gap-2">
            <button 
              type="button"
              onClick={() => messages.length > 0 && speakIt(messages[messages.length-1].it)}
              className="w-12 h-12 rounded-full bg-[#c9a84c]/10 border border-[#c9a84c]/20 flex items-center justify-center text-[#c9a84c] hover:bg-[#c9a84c]/20 transition-all"
            >
              <Volume2 size={20} />
            </button>
            <span className="text-[0.55rem] uppercase tracking-widest text-[#c9a84c]/40">Herhoor</span>
          </div>
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            >
              {isRecording ? <MicOff size={32} className="text-red-500" /> : <Mic size={32} className="text-[#080810]" />}
            </button>
            <span className={`text-[0.65rem] uppercase tracking-widest font-bold ${isRecording ? 'text-red-500' : 'text-[#c9a84c]'}`}>
              {isRecording ? 'Luisteren...' : 'Antwoorden'}
            </span>
          </div>
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-[0.55rem] uppercase tracking-widest text-[#c9a84c]/40">Sla over</span>
          </div>
        </div>

        {/* Status & Score */}
        <div className="text-center space-y-4">
          <p className="text-[0.65rem] text-[#c9a84c]/40 min-h-[1em] italic">{status}</p>
          
          <div className="flex items-center justify-between border-t border-[#c9a84c]/10 pt-4">
            <div className="flex items-center gap-2 text-[#c9a84c]/40 text-[0.6rem] uppercase tracking-widest">
              <Trophy size={12} /> Punteggio
            </div>
          ...
          <button 
            type="button"
            onClick={startNewConversation}
            className="w-full py-3 border border-[#c9a84c]/20 rounded-xl text-[0.7rem] tracking-[0.2em] uppercase text-[#c9a84c]/70 hover:bg-[#c9a84c]/5 hover:border-[#c9a84c]/40 transition-all flex items-center justify-center gap-2"
          >
            <RotateCcw size={14} /> Nieuw Gesprek
          </button>
          ...
          <div className="flex gap-2">
            <button 
              type="button"
              onClick={downloadTranscript}
              className="flex-1 py-2 border border-[#c9a84c]/10 rounded-lg text-[0.6rem] tracking-widest uppercase text-[#c9a84c]/40 hover:text-[#c9a84c]/70 transition-all"
            >
              💾 Opslaan als tekst
            </button>
            <button 
              type="button"
              onClick={() => setShowKeyModal(true)}
              className="px-3 py-2 border border-[#c9a84c]/10 rounded-lg text-[0.6rem] text-[#c9a84c]/40 hover:text-[#c9a84c]/70 transition-all"
            >
              🔑 Key
            </button>
          </div>
          ...
          >
            ...
            >
              ...
              <div className="flex gap-2">
                <button 
                  onClick={() => setShowKeyModal(false)}
                  className="flex-1 py-2 text-xs text-[#c9a84c]/50"
                >
                  Annuleren
                </button>
                <button 
